...
*Setup instructions for a Gitian build of Ripoff Coin using a Debian VM or physical system.*
...
- [Building Ripoff Coin](#building-ripoffcoin-core)
...
Clone the git repositories for Ripoff Coin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ripoffcoinpay/ripoffcoin
```
...
in the Ripoff Coin repository under 'Fetch and build inputs' to install sources which require
...
To build Ripoff Coin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Ripoff Coin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ripoffcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/ripoffcoinpay/ripoffcoin
...
For example:
```bash
URL=https://github.com/crowning-/ripoffcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit ripoffcoin=${COMMIT} --url ripoffcoin=${URL} ../ripoffcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ripoffcoin=${COMMIT} --url ripoffcoin=${URL} ../ripoffcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ripoffcoin=${COMMIT} --url ripoffcoin=${URL} ../ripoffcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the ripoffcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../ripoffcoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/ripoffcoinpay/ripoffcoin-detached-sigs.git

BTCPATH=/some/root/path/ripoffcoin.git
SIGPATH=/some/root/path/ripoffcoin-detached-sigs.git

./bin/gbuild --url ripoffcoin=${BTCPATH},signature=${SIGPATH} ../ripoffcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ripoffcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ripoffcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/ripoffcoin-osx-build.assert
```
...
[ripoffcoin/gitian.sigs](https://github.com/ripoffcoinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
