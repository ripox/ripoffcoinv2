...
#if defined(NDEBUG)
# error "RIPO Core cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in riposhis) are considered zero fee (for relaying, mining and transaction creation) */
CFeeRate minRelayTxFee = CFeeRate(DEFAULT_LEGACY_MIN_RELAY_TX_FEE);

CTxMemPool mempool(::minRelayTxFee);
map<uint256, int64_t> mapRejectedBlocks GUARDED_BY(cs_main);

static const CAmount ZERO_POW_REWARD = COIN * 0;
static const CAmount ONE_POW_REWARD = COIN * 140;
static const CAmount TWO_POW_REWARD = COIN * 70;
static const CAmount THREE_POW_REWARD = COIN * 35;
static const CAmount FOUR_POW_REWARD = COIN * 17.5;
static const CAmount FIVE_POW_REWARD = COIN * 8.75;

// Masternode Payments
static const CAmount ONE_MASTERNODE_PAYMENT = COIN * 49;
static const CAmount TWO_MASTERNODE_PAYMENT = COIN * 24.5;
static const CAmount THREE_MASTERNODE_PAYMENT = COIN * 12.25;
static const CAmount FOUR_MASTERNODE_PAYMENT = COIN * 6.125;
static const CAmount FIVE_MASTERNODE_PAYMENT = COIN * 3.0625;
...
*/
CAmount GetBlockSubsidy(const int &chainHeight, CAmount nFees)
{
    if (chainHeight == 1) {
        CAmount nSubsidy = 1260000 * COIN;
        LogPrint("superblock creation", "GetPoWBlockPayment() : create=%s nSubsidy=%d\n", FormatMoney(nSubsidy), nSubsidy);
        return nSubsidy;
    }
    else if (chainHeight > 1 && chainHeight <= Params().GetConsensus().n0TotalBlocks) {
        LogPrint("creation", "GetPoWBlockPayment() : create=%s PoW Reward=%d\n", FormatMoney(ZERO_POW_REWARD), ZERO_POW_REWARD);
        return ZERO_POW_REWARD + nFees;
    }
    else if (chainHeight > Params().GetConsensus().n0TotalBlocks && chainHeight <= Params().GetConsensus().n1TotalBlocks) {
        LogPrint("creation", "GetPoWBlockPayment() : create=%s PoW Reward=%d\n", FormatMoney(ONE_POW_REWARD), ONE_POW_REWARD);
        return ONE_POW_REWARD + nFees;
    }
    else if (chainHeight > Params().GetConsensus().n1TotalBlocks && chainHeight <= Params().GetConsensus().n2TotalBlocks) {
        LogPrint("creation", "GetPoWBlockPayment() : create=%s PoW Reward=%d\n", FormatMoney(TWO_POW_REWARD), TWO_POW_REWARD);
        return TWO_POW_REWARD + nFees;
    }
    else if (chainHeight > Params().GetConsensus().n2TotalBlocks && chainHeight <= Params().GetConsensus().n3TotalBlocks) {
        LogPrint("creation", "GetPoWBlockPayment() : create=%s PoW Reward=%d\n", FormatMoney(THREE_POW_REWARD), THREE_POW_REWARD);
        return THREE_POW_REWARD + nFees;
    }
    else if (chainHeight > Params().GetConsensus().n3TotalBlocks && chainHeight <= Params().GetConsensus().n4TotalBlocks) {
        LogPrint("creation", "GetPoWBlockPayment() : create=%s PoW Reward=%d\n", FormatMoney(FOUR_POW_REWARD), FOUR_POW_REWARD);
        return FOUR_POW_REWARD + nFees;
    }
    else if (chainHeight > Params().GetConsensus().n4TotalBlocks && chainHeight <= Params().GetConsensus().n5TotalBlocks) {
        LogPrint("creation", "GetPoWBlockPayment() : create=%s PoW Reward=%d\n", FormatMoney(FIVE_POW_REWARD), FIVE_POW_REWARD);
        return FIVE_POW_REWARD + nFees;
    }
    else
        return nFees;
}

CAmount GetMasternodePayment(const int &chainHeight, bool fMasternode)
{
    if (fMasternode && chainHeight > Params().GetConsensus().nMasternodePaymentsStartBlock && chainHeight <= Params().GetConsensus().n1TotalBlocks) {
        LogPrint("creation", "GetMasternodePayment() : create=%s MN Payment=%d\n", FormatMoney(ONE_MASTERNODE_PAYMENT), ONE_MASTERNODE_PAYMENT);
        return ONE_MASTERNODE_PAYMENT;
    }
    else if (fMasternode && chainHeight > Params().GetConsensus().n1TotalBlocks && chainHeight <= Params().GetConsensus().n2TotalBlocks) {
        LogPrint("creation", "GetMasternodePayment() : create=%s MN Payment=%d\n", FormatMoney(TWO_MASTERNODE_PAYMENT), TWO_MASTERNODE_PAYMENT);
        return TWO_MASTERNODE_PAYMENT;
    }
    else if (fMasternode && chainHeight > Params().GetConsensus().n2TotalBlocks && chainHeight <= Params().GetConsensus().n3TotalBlocks) {
        LogPrint("creation", "GetMasternodePayment() : create=%s MN Payment=%d\n", FormatMoney(THREE_MASTERNODE_PAYMENT), THREE_MASTERNODE_PAYMENT);
        return THREE_MASTERNODE_PAYMENT;
    }
    else if (fMasternode && chainHeight > Params().GetConsensus().n3TotalBlocks && chainHeight <= Params().GetConsensus().n4TotalBlocks) {
        LogPrint("creation", "GetMasternodePayment() : create=%s MN Payment=%d\n", FormatMoney(FOUR_MASTERNODE_PAYMENT), FOUR_MASTERNODE_PAYMENT);
        return FOUR_MASTERNODE_PAYMENT;
    }
    else if (fMasternode && chainHeight > Params().GetConsensus().n4TotalBlocks) {
        LogPrint("creation", "GetMasternodePayment() : create=%s MN Payment=%d\n", FormatMoney(FIVE_MASTERNODE_PAYMENT), FIVE_MASTERNODE_PAYMENT);
        return FIVE_MASTERNODE_PAYMENT;
    }
    else {
        return 0;
    }
...
void CheckForkWarningConditions()
{
    ...
    // If our best fork is no longer within 90 blocks (+/- 3 hours if no one mines it)
    // of our head, drop it
    if (pindexBestForkTip && chainActive.Height() - pindexBestForkTip->nHeight >= 90)
...
void ThreadScriptCheck() {
    RenameThread("ripoffcoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // RIPO : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    CAmount blockReward = GetBlockSubsidy(pindex->pprev->nHeight, nFees);
    std::string strError = "";
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(RIPO): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(RIPO): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // RIPO : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(RIPO): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(RIPO): spork is off, skipping transaction locking checks\n");
    }
