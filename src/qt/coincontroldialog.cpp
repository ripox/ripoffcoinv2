        ...
        if (item->checkState(COLUMN_CHECKBOX) == Qt::Unchecked)
            ...
            if (coinControl->fUsePrivateSend && nRounds < privateSendClient.nPrivateSendRounds) {
                QMessageBox::warning(this, windowTitle(),
                    tr("Non-anonymized input selected. <b>DDNSend will be disabled.</b><br><br>If you still want to use DDNSend, please deselect all non-nonymized inputs first and then check DDNSend checkbox again."),
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::RIPO;
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            if(ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show ripoffcoin address. In tree mode, address is not shown again for direct wallet address outputs
