...
public:
    TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(parent), unit(BitcoinUnits::RIPO),
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    {
        // update the display unit, to not use the default ("RIPO")
        updateDisplayUnit();
...
void OverviewPage::privateSendStatus()
{
    ...
    if (nWalletBackups > 0 && pwalletMain->nKeysLeftSinceAutoBackup < PRIVATESEND_KEYS_THRESHOLD_WARNING) {
        ...
        if(settings.value("fLowKeysWarning").toBool()) {
            ...
            QMessageBox::warning(this, tr("DDNSend"), strWarn, QMessageBox::Ok, QMessageBox::Ok);
        } else {
        ...
        if(!AutoBackupWallet(pwalletMain, "", strBackupWarning, strBackupError)) {
            if (!strBackupWarning.empty()) {
                ...
                QMessageBox::warning(this, tr("DDNSend"),
    ...
    QString s = tr("Last DDNSend message:\n") + strStatus;

    if(s != ui->labelPrivateSendLastMessage->text())
        LogPrintf("OverviewPage::privateSendStatus -- Last DDNSend message: %s\n", strStatus.toStdString());
...
void OverviewPage::privateSendReset(){
    ...
    QMessageBox::warning(this, tr("DDNSend"),
        tr("DDNSend was successfully reset."),
        QMessageBox::Ok, QMessageBox::Ok);
}
...
void OverviewPage::togglePrivateSend(){
    ...
    if(hasMixed.isEmpty()){
        QMessageBox::information(this, tr("DDNSend"),
                tr("If you don't want to see internal DDNSend fees/transactions select \"Most Common\" as Type on the \"Transactions\" tab."),
                QMessageBox::Ok, QMessageBox::Ok);
    ...
    if(!privateSendClient.fEnablePrivateSend){
        ...
        if(currentBalance < nMinAmount){
            ...
            QMessageBox::warning(this, tr("DDNSend"),
                tr("DDNSend requires at least %1 to use.").arg(strMinAmount),
                QMessageBox::Ok, QMessageBox::Ok);
            return;
        }
        ...
        if (walletModel->getEncryptionStatus() == WalletModel::Locked)
        {
            ...
            if(!ctx.isValid())
            {
                ...
                QMessageBox::warning(this, tr("DDNSend"),
                    tr("Wallet is locked and user declined to unlock. Disabling DDNSend."),
                    QMessageBox::Ok, QMessageBox::Ok);
                LogPrint("privatesend", "OverviewPage::togglePrivateSend -- Wallet is locked and user declined to unlock. Disabling DDNSend.\n");
                return;
            }
        }
