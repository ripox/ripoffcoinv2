    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("RIPO Core") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About RIPO Core"));
        ...
        QString header = tr("Usage:") + "\n" +
            "  ripoffcoin-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        setWindowTitle(tr("DDNSend information"));

        ui->aboutMessage->setTextFormat(Qt::RichText);
        ui->scrollArea->setVerticalScrollBarPolicy(Qt::ScrollBarAsNeeded);
        ui->aboutMessage->setText(tr("\
<h3>DDNSend Basics</h3> \
DDNSend gives you true financial privacy by obscuring the origins of your funds. \
All the RIPO in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
DDNSend uses an innovative process to mix your inputs with the inputs of two other people, without having your coins ever leave your wallet. \
You retain control of your money at all times..<hr> \
<b>The DDNSend process works like this:</b>\
<ol type=\"1\"> \
<li>DDNSend begins by breaking your transaction inputs down into standard denominations. \
These denominations are 0.01 RIPO, 0.1 RIPO, 1 RIPO and 10 RIPO -- sort of like the paper money you use every day.</li> \
...
Each time the process is completed, it's called a \"round.\" Each round of DDNSend makes it exponentially more difficult to determine where your funds originated.</li> \
...
Consequently, users who have backups disabled will also have DDNSend disabled. <hr>\
For more info see <a href=\"https://ripoffcoinpay.atlassian.net/wiki/display/DOC/DDNSend\">https://ripoffcoinpay.atlassian.net/wiki/display/DOC/DDNSend</a> \
        "));
        ui->aboutMessage->setWordWrap(true);
        ui->helpMessage->setVisible(false);
        ui->aboutLogo->setVisible(false);
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("RIPO Core is shutting down...") + "<br /><br />" +
