...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("RIPO Core RPC client version") + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  ripoffcoin-cli [options] <command> [params]  " + _("Send command to RIPO Core") + "\n" +
                  "  ripoffcoin-cli [options] help                " + _("List commands") + "\n" +
                  "  ripoffcoin-cli [options] help <command>      " + _("Get help for a command") + "\n";
