...
const char * const BITCOIN_CONF_FILENAME = "ripoffcoin.conf";
const char * const BITCOIN_PID_FILENAME = "ripoffcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "ripoffcoin" is a composite category enabling all RIPO-related debug output
            if(ptrCategory->count(string("ripoffcoin"))) {
...
#else
    const char* pszModule = "ripoffcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RipOffCoinV2Core
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RipOffCoinV2Core
    // Mac: ~/Library/Application Support/RIPOCore
    // Unix: ~/.ripoffcoinv2
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RipOffCoinV2Core";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RipOffCoinV2Core";
#else
    // Unix
    return pathRet / ".ripoffcoinv2";
#endif
#endif
}
...
static void WriteConfigFile(FILE* configFile)
{
    ...
    fputs ("rpcport=37005\n", configFile);
    fputs ("port=37020\n",configFile);
//    fputs ("addnode=80.209.228.109\n",configFile);
//    fputs ("addnode=80.209.228.110\n",configFile);
    fclose(configFile);
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty ripoffcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override ripoffcoin.conf
