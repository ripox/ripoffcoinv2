...
/** 
 * Fees smaller than this (in riposhis) are considered zero fee (for transaction creation)
...
// Recursively determine the rounds of a given input (How deep is the DDNSend chain for a given input)
...
bool CWallet::CreateCollateralTransaction(CMutableTransaction& txCollateral, std::string& strReason)
{
    ...
    if (!GetCollateralTxIn(txinCollateral, nValue)) {
        strReason = "DDNSend requires a collateral transaction and could not locate an acceptable input!";
        return false;
    }
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseInstantSend))
                {
                    if (nCoinType == ONLY_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 RIPO.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough DDNSend non-denominated funds for this transaction that are not equal 1000 RIPO.");
                    } else if (nCoinType == ONLY_DENOMINATED) {
                        strFailReason = _("Unable to locate enough DDNSend denominated funds for this transaction.");
                        strFailReason += " " + _("DDNSend uses exact denominated amounts to send funds, you might simply need to anonymize some more coins.");
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 RIPO."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-ripoffcoin-address
                        CScript scriptChange;
