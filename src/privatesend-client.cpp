...
void CPrivateSendClient::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all RIPO related functionality
    ...
    if(strCommand == NetMsgType::DSQUEUE) {
        ...
        if(dsq.fReady) {
            ...
            if(nState == POOL_STATE_QUEUE) {
                LogPrint("privatesend", "DSQUEUE -- DDNSend queue (%s) is ready on masternode %s\n", dsq.ToString(), infoMn.addr.ToString());
            ...
            LogPrint("privatesend", "DSQUEUE -- new DDNSend queue (%s) from masternode %s\n", dsq.ToString(), infoMn.addr.ToString());
        ...
        if(nSessionID != nMsgSessionID) {
            LogPrint("privatesend", "DSFINALTX -- message doesn't match current DDNSend session: nSessionID: %d  nMsgSessionID: %d\n", nSessionID, nMsgSessionID);
            return;
        }
        ...
        if(nSessionID != nMsgSessionID) {
            LogPrint("privatesend", "DSCOMPLETE -- message doesn't match current DDNSend session: nSessionID: %d  nMsgSessionID: %d\n", nSessionID, nMsgSessionID);
            return;
        }
    ...
    switch(nState) {
        ...
        case POOL_STATE_ACCEPTING_ENTRIES:
            if(nEntriesCount == 0) {
                ...
                return _("DDNSend request complete:") + " " + _("Your transaction was accepted into the pool!");
            } else {
        ...
        case POOL_STATE_ERROR:
            return _("DDNSend request incomplete:") + " " + strLastMessage + " " + _("Will retry...");
        case POOL_STATE_SUCCESS:
            return _("DDNSend request complete:") + " " + strLastMessage;
       default:
...
{
    if(fMasterNode) {
        LogPrintf("CPrivateSendClient::SendDenominate -- DDNSend from a Masternode is not supported currently.\n");
        return false;
    }

    if(txMyCollateral == CMutableTransaction()) {
        LogPrintf("CPrivateSendClient:SendDenominate -- DDNSend collateral not set\n");
        return false;
    }
    ...
    if(!CheckDiskSpace()) {
        ...
        LogPrintf("CPrivateSendClient::SendDenominate -- Not enough disk space, disabling DDNSend.\n");
        return false;
    }
...
bool CPrivateSendClient::DoAutomaticDenominating(CConnman& connman, bool fDryRun)
{
    ...
    if(WaitForAnotherBlock()) {
        LogPrintf("CPrivateSendClient::DoAutomaticDenominating -- Last successful DDNSend action was too recent\n");
        strAutoDenomResult = _("Last successful DDNSend action was too recent.");
        return false;
    }
...
bool CPrivateSendClient::SubmitDenominate(CConnman& connman)
{
    ...
    for(int i = nPrivateSendRounds; i > 0; i--) {
        if(PrepareDenominate(i - 1, i, strError, vecTxInRet, vecTxOutRet)) {
            LogPrintf("CPrivateSendClient::SubmitDenominate -- Running DDNSend denominate for %d rounds, success\n", i);
            return SendDenominate(vecTxInRet, vecTxOutRet, connman);
        }
        LogPrint("privatesend", "CPrivateSendClient::SubmitDenominate -- Running DDNSend denominate for %d rounds, error: %s\n", i, strError);
    }

    // We failed? That's strange but let's just make final attempt and try to mix everything
    if(PrepareDenominate(0, nPrivateSendRounds, strError, vecTxInRet, vecTxOutRet)) {
        LogPrintf("CPrivateSendClient::SubmitDenominate -- Running DDNSend denominate for all rounds, success\n");
        return SendDenominate(vecTxInRet, vecTxOutRet, connman);
    }

    // Should never actually get here but just in case
    LogPrintf("CPrivateSendClient::SubmitDenominate -- Running DDNSend denominate for all rounds, error: %s\n", strError);
...
bool CPrivateSendClient::PrepareDenominate(int nMinRounds, int nMaxRounds, std::string& strErrorRet, std::vector<CTxIn>& vecTxInRet, std::vector<CTxOut>& vecTxOutRet)
{
    ...
    if (GetEntriesCount() > 0) {
        strErrorRet = "Already have pending entries in the DDNSend pool";
        return false;
    }
...
{
    if(fLiteMode) return; // disable all RIPO specific functionality

    static bool fOneThread;
    if(fOneThread) return;
    fOneThread = true;

    // Make this thread recognisable as the DDNSend thread
    RenameThread("ripoffcoin-ps-client");
